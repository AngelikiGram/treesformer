...
class FullMultimodalEncoder(nn.Module):
    ...
    def __init__(self, dim=512, visual_bottleneck=640, visual_dropout=0.05, num_species=30):
        ...
        # 3. Balanced Global Fusion -> Species Head
        # Moves species logic inside the perception stack
    ...
    def forward(self, dsm_pts, ortho_img):
        ...
        if not ortho_is_missing:
            ...
            o = torch.zeros((B, 196, self.fuse.in_features), device=device)

        # 2. Process DSM if available
        if not dsm_is_missing:
            d = self.dsm_backbone(dsm_pts)
        else:
            d = torch.zeros((B, dsm_pts.shape[1], self.fuse.in_features), device=device)
        
        # --- BALANCED GLOBAL FUSION (Equal Attention Policy) ---
        # Instead of concatenation, we use shared-space addition.
        # This forces the model to learn a unified representation of the tree 
        # where DSM and Ortho are just two "views" of the same latent content.
        d_global = torch.max(d, dim=1)[0]
        o_global = torch.max(o, dim=1)[0]
        ...
        return visual_memory, global_feat, species_logits
...
class LSystemModel(nn.Module):
    ...
    def pure_inference(self, dsm_pts, ortho_img, tokenizer, max_len=256, temperature=1.0, temperature_structural=0.7, max_depth=20):
        ...
        visual_memory, global_feat, species_logits = self.mm(dsm_pts, ortho_img)
        self._pooled_visual_cache = species_logits
    ...
    def forward(self, type_in, val_in, dsm_pts=None, ortho_img=None, kv_caches=None, pos_offset=0, visual_memory_cache=None):
        ...
        if visual_memory_cache is not None:
            visual_memory, global_feat, species_logits = visual_memory_cache
        else:
            # Species prediction is now internal to the multimodal encoder
            visual_memory, global_feat, species_logits = self.mm(dsm_pts, ortho_img)
        
        self._pooled_visual_cache = species_logits  # Cache logits instead of features
...
def forward_with_truncated_bptt(model, type_in, val_in, dsm_pts, ortho_img, chunk_size=2048, visual_memory_cache=None):
    ...
    ...
    Args:
        model: LSystemModel instance
        type_in: (B, T_total) full sequence of token types
        val_in: (B, T_total, 3) full sequence of token values
        dsm_pts, ortho_img: visual inputs (ignored if visual_memory_cache is provided)
        chunk_size: Size of each chunk (default: 2048)
        visual_memory_cache: Pre-computed visual features
        
    Returns:
        all_type_logits, all_val_logits, species_logits, all_pred_states
    """
    ...
    # ✅ TRUNCATED BPTT: Reuse visual memory across chunks (computed once per sequence)
    if visual_memory_cache is not None:
        visual_memory = visual_memory_cache
    else:
        visual_memory = model.mm(dsm_pts, ortho_img)
    ...
    all_val_logits = []
    all_pred_states = []
    species_logits = None  # Computed from last chunk
        ...
        for batch_idx, batch in enumerate(tqdm(loader, desc=f"Epoch {ep}")):
            ...
            with torch.amp.autocast('cuda'):
                if use_truncated_bptt and t_in.shape[1] > bptt_chunk_size:
                    ...
                    all_tlog, all_vlog, sp_logits, all_pred_states = forward_with_truncated_bptt(
                    ...
                    # Passing None, None explicitly to avoid any signature inspection issues
                    tlog, vlog, sp_logits, _, _, pred_state = model(
                        t_in, v_in, None, None, visual_memory_cache=v_mem
                    )
                    # sp_logits is already computed internally in the model
